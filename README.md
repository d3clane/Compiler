...
Среди опциональных флагов backend на данный момент есть только `-S`, который аналогичен такому же в `gcc`, то есть включает создание ассемблерного файла с кодом.
...
3. Backend переводит AST в исполняемый бинарный файл. В данном случае в проекте представлено два различных backend'а - для нативной архитектуры x86_64 и моего эмулятора процессора.
...
Если же писать монолитные программы, то потребуется $nm$ таких программ. Очевидно, что первое предпочтительнее, даже несмотря на то, что появятся лишние издержки на запись и чтение дерева из файла(временного хранилища между программами).
...
<img src="https://github.com/d3clane/Compiler/blob/main/ReadmeAssets/imgs/StdlibElfHeader.png" width="20%" height="40%">
